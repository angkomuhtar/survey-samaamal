                ...
                <div class="p-6">
                  <form class="space-y-4" id="sending_form">
                      ...
                      <div class="input-area relative">
                          <label for="largeInput" class="form-label">Work Hours Tipe</label>
                          <div class="relative">
                              <select id="select" class="form-control !pl-9" name="shift">
                                <option value="" selected disabled class="dark:bg-slate-700 text-slate-300">Pilih Data</option>
                                @foreach ($shift as $item)
                                <option value="{{$item->id}}" class="dark:bg-slate-700">{{$item->shift}}</option>
                                @endforeach
                              </select>
                      ...
                      <div class="input-area relative">
                        <label for="largeInput" class="form-label">Start Works</label>
                        <div class="relative">
                          <input class="form-control py-2 flatpickr time flatpickr-input active !pl-9" id="time-picker" name="start" value="" type="text" readonly="readonly">
                      ...
                      <div class="input-area relative">
                        <label for="largeInput" class="form-label">End Works</label>
                        <div class="relative">
                          <input class="form-control py-2 flatpickr time flatpickr-input active !pl-9" id="time-picker" name="end" value="" type="text" readonly="readonly">
                          <iconify-icon icon="heroicons:globe-alt" class="absolute left-2 top-1/2 -translate-y-1/2 text-base text-slate-500"></iconify-icon>
                        </div>
                      </div>
                      <div class="input-area relative">
                        <label for="largeInput" class="form-label">Shift Name</label>
                        <div class="relative">
                          <input type="text" name="name" class="form-control !pl-9" placeholder="Name Shift">
                          <iconify-icon icon="heroicons:globe-alt" class="absolute left-2 top-1/2 -translate-y-1/2 text-base text-slate-500"></iconify-icon>
                        </div>
                      </div>
    ...
    <script type="module">
      ...
      // submit data
        $(document).on('submit', '#sending_form', (e)=>{
          ...
          var url = type == 'submit' ? '{!! route('absensi.workhours.store') !!}' : '{!! route('absensi.workhours.update', ['id'=>':id']) !!}';
        ...
        $(document).on('click', '#btn-delete', (e)=>{
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              var url = '{!! route('absensi.workhours.destroy', ['id'=>':id']) !!}';
              url = url.replace(':id', id);
              $.ajax({
                url : url,
                type : 'DELETE',
